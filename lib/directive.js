...
function collectImage ($img) {
  ...
  pool[idx] = { src: originSrc || src, msrc: src, title, w, h }
  if (scope) {
  ...
  return {
    inserted (el, binding, vnode, oldVnode) {
      ...
      if (typeof value === 'object') {
        Object.assign(events, value.events || {})
        el._scope = value.name || scope
        el._events = events
        el._options = { ...options, events }
      }
